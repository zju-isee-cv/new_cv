...
%   I : calibration image
%   kk : image index
%   gen_KK_est : calibration matrix
%   gridInfo : structure containing the grid points and images
%              (see "click_calib.m")
%   paramEst : estimate of the extrinsic parameters 
%              (all the parameters later in the calibratin process)
%
% Output :
%   Adds the grid points to 'gridInfo' and an estimate of the
%   extrinsic parameters to 'paramEst'.
%   errorExtr : 1 if the extraction did not go well well, 0 otherwise
%
function [gridInfo,paramEst,errorExtr] = click_ima_calib(I,kk,gen_KK_est,gridInfo,paramEst)
...
for i=1:4
  if i==4
    P = omniLinePoints([x(4),x(1);y(4),y(1);1,1],gen_KK_est,paramEst.xi);
  else
    P = omniLinePoints([x(i:i+1)';y(i:i+1)';1,1],gen_KK_est,paramEst.xi);
  end
  plot(P(1,:),P(2,:),'g-');
end
...
V = [1;0.001*ones(6,1);paramEst.xi;zeros(5,1);0;
...
pts = rigid_motion_quat(grid3D,Qw,Tw);

%Project all points according to equations
XX = spaceToImgPlane(pts, [paramEst.xi;0;0;0;0;0;0;gen_KK_est(1,1);...
		    gen_KK_est(2,2);gen_KK_est(1:2,3)]);


Np = (n_sq_x+1)*(n_sq_y+1);

fprintf(1,'Corner extraction... ');
...
gridInfo.x{kk} = grid_pts+1; % Use Matlab convention
