...
function [Xk,error,k]=levenbergMarquardt6(gxf,gxj,Xinput,X0,Xoutput)
...
sfx = feval(gxf,Xinput,X0,Xoutput);
Jx = feval(gxj,Xinput,X0);
...
while not(found) & (k<kmax)
  k=k+1;
  [Xk,sfx,A,g,mu,nu,found]=mainIteration(gxf,gxj,Xinput,Xoutput,Xk,sfx,A,g,emax1,emax2,cond_thresh,mu,nu,nbParam);
end

error=norm(feval(gxf,Xinput,Xk,Xoutput));
...
[X0,error,k]=levenbergMarquardt6(@fitexp_f,@fitexp_j,Xinput,X0,Xoutput);
...
function f=fitexp_f(Xinput, Xk, Xoutput)
E=exp(Xinput*[Xk(1),Xk(2)]);
f=E*[Xk(3);Xk(4)]-Xoutput;

function J=fitexp_j(Xinput, Xk)
E=exp(Xinput*[Xk(1),Xk(2)]);
...
function [Xk,sfx,A,g,mu,nu,found]=mainIteration(gxf,gxj,Xinput,Xoutput,Xk,sfx,A,g,emax1,emax2,cond_thresh,mu,nu,nbParam)
...
if 0%norm(hlm)<=emax2*(norm(Xk)+emax2)
  ...
  Xkp1=Xk+hlm;
  sfxp1=feval(gxf,Xinput,Xkp1,Xoutput);
  sFx=norm(sfx)^2;
  sFxp1=norm(sfxp1)^2;
  ...
  if quote>0
    Xk=Xkp1;
    sfx=sfxp1;
    sfx = feval(gxf,Xinput,Xk,Xoutput);
    Jx = feval(gxj,Xinput,Xk);
    A=Jx'*Jx;
    g=Jx'*sfx;
    found=max(abs(g))<emax1;
