...
cell_list{3,1} = {'Extract grid corners manually',['check_click_calib;'...
		    'if ~missing '...
		    '[gridInfo,paramEst, paramEst3D]  ='...
		    'extract_grid_corners_manually(images,gen_KK_est,gridInfo,paramEst, paramEst3D);'...
		    'end']};

cell_list{3,2} = {'Change_corners',['check_click_calib;'...
		    'if ~missing '...
		    'gridInfo ='...
		    'change_corners(gridInfo);'...
		    'end']};

cell_list{3,3} = {'Initialize RT',['check_click_calib;'...
		    'if ~missing '...
		    '[paramEst, paramEst3D] ='...
		    'estallRT(paramEst, gridInfo, gen_KK_est, paramEst3D);'...
		    'end']};

cell_list{3,4} = {'Assign RTest',['check_click_calib;'...
		    'if ~missing '...
		    '[paramEst, paramEst3D] ='...
		    'AssignRTest(paramEst, paramEst3D);'...
		    'end']};
        
cell_list{4,1} = {'Calibration of two methods',['check_calib_optim;biased_calib=1;' ...
		    'if ~missing '...
		    '[paramEst, images, paramEst3D] = go_omni_calib_optim_iter3D(minInfo,images,gen_KK_est,gridInfo,paramEst, paramEst3D);'...
		    'end']};
cell_list{4,2} = {'Calib new RT',['check_calib_optim;biased_calib=1;' ...
		    'if ~missing '...
		    '[gridInfo, paramEst, paramEst3D] = calib_new_RT(images, gen_KK_est, gridInfo, paramEst, paramEst3D);'...
		    'end']};      
 cell_list{4,3} = {'Analysis_angle',['check_click_calib;'...
		    'if ~missing '...
		    'data_analysis(paramEst, paramEst3D);'...
		    'end']};
        
 cell_list{4,4} = {'Show calib results','show_calib_results(images,gen_KK_est,paramEst,gridInfo, paramEst3D);'};
 cell_list{5,1} = {'Save','save_omni_calib(minInfo,borderInfo,images,gen_KK_est,gridInfo,paramEst, paramEst3D);'};
 cell_list{5,2} = {'Load','load_omni_calib;'};
 cell_list{5,3} = {'Exit',['disp(''Bye. To run again, type omni_calib_gui.''); close(' num2str(fig_number) ');']}; %{'Exit','calib_gui;'};
